...
std::shared_ptr<VectorDataTyped<DTYPE>> VectorDataTyped<DTYPE>::create(
    const std::string& path, std::shared_ptr<AQNWB::IO::BaseIO> io)
{
  if (io) {
    ...
    if (recordingObjects) {
      ...
      if (existingObj) {
        auto typedObj =
            std::dynamic_pointer_cast<VectorDataTyped<DTYPE>>(existingObj);
        if (typedObj) {
          return typedObj;
        }
        // If an object with the same path exists but is not of this exact type
        // (e.g., it's a base VectorData), we create a new VectorDataTyped
        // instance but do NOT add it to the recording objects cache to avoid
        // overwriting the existing VectorDataobject or causing conflicts.
