...
* Added new `EventsTable` type to support recording of event data. (@cline, @oruebel, [#304](https://github.com/NeurodataWithoutBorders/aqnwb/pull/304))
...
* Added `DynamicTable::addRow` and `DynamicTable::addRows` methods to support row-based data insertion into a `DynamicTable`. Rows are specified as `RowData` (an `unordered_map` from column name to `CellValue` variant), enabling type-safe, column-keyed writes without requiring callers to manage per-column buffers directly. Row IDs are auto-generated if not provided. (@cline, @oruebel, [#305](https://github.com/NeurodataWithoutBorders/aqnwb/pull/305)). 
    * Enhanced the new `CellValue` to support both scalar and vector data, enabling support for ragged array columns. (@cline, @oruebel, [#325](https://github.com/NeurodataWithoutBorders/aqnwb/pull/325))
    * Moved `CellValue` and `RowData` definitions to `Types.hpp` to avoid circular dependencies, and moved `VectorData` implementation details to `VectorData.cpp`. (@cline, @oruebel, [#325](https://github.com/NeurodataWithoutBorders/aqnwb/pull/325))
    * Updated `DynamicTable::addRows` and related methods to support ragged array column writes. This also included moving of `CellValue` and `RowData` definitions to `Types.hpp`; enhancement of `CellValue` to support scalar and vector types for ragged columns; and addition of `VectorData::appendData` and `VectorIndex::appendData` (@cline, @oruebel, [#325](https://github.com/NeurodataWithoutBorders/aqnwb/pull/325))
...
* Added `DynamicTable::validateDataSpecs` (and `DynamicTable::checkRequiredColumnNames` helper function) to validate column specifications as part of `DynamicTable::initialize` before initialization. Also updated subclassed of `DynamicTable` (e.g., `ElectrodesTable`, `EventsTable`, `MeaningsTable`) to override `validateDataSpecs` to provide their specific validation logic. (@oruebel, [#305](https://github.com/NeurodataWithoutBorders/aqnwb/pull/305))
...
* **[BREAKING]** Updated `RegisteredType::create` to retrieve objects from the `RecordingObjects` cache if they already exist, rather than creating a new instance. This ensures that multiple requests for the same object path return the exact same object instance. This change is mostly transparent to users, but it centralizes the recording state. As part of this, the `BaseRecordingData` cache has been moved from `RegisteredType` to `BaseIO`. Users who were manually accessing or clearing the cache via `RegisteredType::getCacheRecordingData()` or `RegisteredType::clearRecordingDataCache()` will now need to use `BaseIO::getRecordingDataCache()` and `BaseIO::clearRecordingDataCache()` instead. (@cline, @oruebel, [#328](https://github.com/NeurodataWithoutBorders/aqnwb/pull/328))
...
* **[BREAKING]** Updated `Types.hpp` to use a `namespace` instead of `class` to group types. For most users this change should not require any code changes since the syntax for accessing the types is the same (e.g., `AQNWB::Types::Status`), but for users who were using `using` declarations to import the type can now use `using namespace AQNWB::Types`. (@oruebel, [#325](https://github.com/NeurodataWithoutBorders/aqnwb/pull/325))
* Updated `DynamicTable::initialize`, `ElectrodesTable::initialize`, `EventsTable::initialize`, and `MeaningsTable::initialize` to accept a `std::vector<DataSpecPtr>` column-spec list instead of individual `rowChunkSize` / resolution / flag parameters. Callers should use the corresponding `createDefaultDataSpecs(...)` static factory to obtain the default spec vector, optionally modify it, and pass it to `initialize()`. `NWBFile::createElectrodesTable` and `NWBFile::createEventsTable` have been updated accordingly. (@cline, @oruebel, [#305](https://github.com/NeurodataWithoutBorders/aqnwb/pull/305))
