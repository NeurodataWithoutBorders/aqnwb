...
* Fixed `cppcheck` `virtualCallInConstructor` warning: `~HDF5IO()` was calling the virtual `close()` from its destructor (@copilot, @oruebel [#278](https://github.com/NeurodataWithoutBorders/aqnwb/pull/278)): extracted the HDF5-specific file-close logic into a new private non-virtual `HDF5IO::closeFileImpl()`; the destructor now calls `BaseIO::close()` (clears recording containers) and `closeFileImpl()` (closes the HDF5 file) directly, with no virtual dispatch
