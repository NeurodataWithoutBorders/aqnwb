...
### Changed
* Standardized naming of `initialize()` parameters generated by `schematype_to_aqnwb` utility (@copilot, [#261](https://github.com/NeurodataWithoutBorders/aqnwb/pull/261)):
    * Parameter names now use consistent lowerCamelCase throughout (e.g., `session_description` → `sessionDescription`)
    * Parent group name is placed first to reflect the NWB file path order (e.g., `experimenterGeneral` → `generalExperimenter`)
    * Unnamed typed fields follow the same parent-first convention (e.g., `paramNWBDataInterfaceAcquisition` → `acquisitionParamNWBDataInterface`)
    * Added `lower_camel` parameter to `snake_to_camel()` utility function to support lowerCamelCase conversion without a separate helper
