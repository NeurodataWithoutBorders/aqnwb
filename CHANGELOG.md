...
* Added `LinkArrayDataSetConfig::validateTarget()` to validate that a link target is compliant with expected schema requirements (@copilot, @oruebel [#259](https://github.com/NeurodataWithoutBorders/aqnwb/pull/259))
  * Checks target existence, data type, dimensionality, and required attributes
  * Used by `TimeSeries::initialize()` to require `conversion`, `resolution`, `offset`, and `unit` on the target dataset
  * Used by `Data::initialize()` to require `namespace`, `object_id`, and `neurodata_type` on the target dataset
  * Used by `VectorData::initialize()` to require `description` on the target dataset
