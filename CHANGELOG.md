...
* Enhanced the `schematype_to_aqnwb` utility script:
    ...
    * Refactored code to generated initalize methods to centralize logic and make the code more robust. E.g., created new `get_initalize_method_parameters` to compile parameter details in one place and split the  `render_initalize_method`  into two functions. (@oruebel, [#230](https://github.com/NeurodataWithoutBorders/aqnwb/pull/230))
