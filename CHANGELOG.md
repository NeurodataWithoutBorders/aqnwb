...
* Added support for creating soft-links to existing datasets to avoid data duplication (@copilot, @oruebel [#257](https://github.com/NeurodataWithoutBorders/aqnwb/pull/257))
  ...
  * Updated `BaseIO::createArrayDataSet` to raise `std::runtime` error on failure (rather than returning   nullptr) to make error handling more robust and to allow link creation to return nullptr without ambiguity.
