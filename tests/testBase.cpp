...
TEST_CASE("TimeSeries", "[base]")
{
  ...
  SECTION("test writing timeseries data block") {
    ...
    io->open();
    NWB::TimeSeries ts = NWB::TimeSeries(dataPath, io, dataType, timestampsType, "unit");
    ts.initialize();
    ...
    static_cast<HDF5::HDF5RecordingData*>(tsDset)->readDataBlock(timestampsType, tsBuffer);
    ...
    static_cast<HDF5::HDF5RecordingData*>(dataDset)->readDataBlock(dataType, dataBuffer);
