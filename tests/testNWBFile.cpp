...
TEST_CASE("createMultipleEcephysDatasets", "[nwb]")
{
  ...
  Status resultCreateSES = nwbfile->createSpikeEventSeries(
      mockArrays, mockSpikeChannelNames, BaseDataType::F32, containerIndices);
...
TEST_CASE("ProcessingModule registered", "[nwb]")
{
  auto registry = AQNWB::NWB::RegisteredType::getRegistry();
  REQUIRE(registry.find("core::ProcessingModule") != registry.end());
}

TEST_CASE("createProcessingModule", "[nwb]")
{
  ...
  auto processingModule =
      nwbfile->createProcessingModule("test_module", "A test processing module");
...
TEST_CASE("createMultipleProcessingModules", "[nwb]")
{
  ...
  // create two ProcessingModules
  auto module1 =
      nwbfile->createProcessingModule("module1", "First processing module");
  REQUIRE(module1 != nullptr);

  auto module2 =
      nwbfile->createProcessingModule("module2", "Second processing module");
