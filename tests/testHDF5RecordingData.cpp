...
TEST_CASE("HDF5RecordingData write operations", "[hdf5recordingdata]")
{
  ...
  SECTION("Write numeric data types")
  {
    // Test int32
    {
      ...
      Status status = dataset->writeDataBlock(SizeArray {5},
                                              SizeArray {0},
                                              BaseDataType::I32,
                                              data.data());
  ...
  SECTION("Write string data")
  {
    // Test fixed-length strings
    {
      ...
      Status status = dataset->writeDataBlock(
          SizeArray {3}, SizeArray {0}, strType, data);
      REQUIRE(status == Status::Success);
    }
    ...
    // Test variable-length strings
    {
      ...
      Status status = dataset->writeDataBlock(
          SizeArray {5}, SizeArray {0}, strType, data);
      REQUIRE(status == Status::Success);
    }
  }
  ...
  SECTION("Error cases")
  {
    // Test dimension mismatch
    {
      ...
      Status status = dataset->writeDataBlock(
          SizeArray {5, 1},  // 2D shape for 1D dataset
          SizeArray {0},
          BaseDataType::I32,
          data.data());
          ...
          "[hdf5recordingdata]")
{
  ...
  SECTION("2D operations")
  {
    ...
    IO::ArrayDataSetConfig config(
        BaseDataType::I32, SizeArray {2, 3}, SizeArray {2, 3});
    auto dataset = hdf5io->createArrayDataSet(config, "/2dDataset");

    // Write full 2D block
    Status status = dataset->writeDataBlock(SizeArray {2, 3},
                                            SizeArray {0, 0},
                                            BaseDataType::I32,
                                            data.data());
    REQUIRE(status == Status::Success);
    ...
    status = dataset->writeDataBlock(SizeArray {1, 2},
                                     SizeArray {1, 1},
                                     BaseDataType::I32,
                                     partial.data());
    REQUIRE(status == Status::Success);
  ...
  SECTION("3D operations")
  {
    ...
    IO::ArrayDataSetConfig config(
        BaseDataType::I32, SizeArray {2, 3, 4}, SizeArray {2, 3, 4});
